...
//!
//! Pairs with `examples/mcxa5xx/src/bin/i3c-controller-dma-64k.rs`
//! (or either-side mate).
//!
//! Per iteration:
...
// BBQ ring backing: 2 * max_rx_transaction so the IRQ can rotate
// grants while the consumer drains the previous one.
...
// TX pattern lives in flash; eDMA reads it directly.
...
async fn main(_spawner: Spawner) {
    ...
    let tgt = target::I3c::new_dma(
        p.I3C0, p.P1_9, p.P1_8, p.DMA0_CH0, p.DMA0_CH1, Irqs, rx_buf, RX_CHUNK, tgt_cfg,
    )
    .unwrap();
    let mut tgt = tgt;
