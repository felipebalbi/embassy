...
use crate::pac::i3c::{Evdet, Ibidis, Mstena, SctrlEvent, SstatusStart, SstatusTxnotfull, Stnotstop, Type};
...
impl<'d, M: Mode> I3c<'d, M> {
    ...
    fn set_configuration(&self, config: &Config) -> Result<(), SetupError> {
        ...
        self.info.regs().sconfig().write(|w| {
            ...
            w.set_s0ignore(true);
            w.set_ddrok(true);
            w.set_bamatch(((self.freq / 1_000_000).max(1) as u8).min(63));
        });
...
impl<'d> I3c<'d, Async> {
    /// Create a new asynchronous instance of the I3C Target bus driver.
    ///
    /// This function initializes the I3C target driver in asynchronous mode. It configures the
    /// I3C peripheral, sets up the clock, and prepares the pins for operation. Any external
...
impl<'d> I3c<'d, Dma<'d>> {
    /// Create a new asynchronous instance of the I3C Target bus driver with DMA support.
    ///
    /// This function initializes the I3C target driver in asynchronous mode with DMA support.
    /// It configures the I3C peripheral, sets up the clock, and prepares the pins for operation.
    ...
    async fn async_respond_to_read_internal<'a>(&'a mut self, buf: &'a [u8]) -> Result<ReadStatus, IOError> {
        ...
        if let Some((last, data)) = buf.split_last() {
            for byte in data.iter() {
                ...
                let status = self.info.regs().sstatus().read();
                if status.stnotstop() == Stnotstop::Stopped || status.start() == SstatusStart::StartDetected {
                    #[cfg(feature = "defmt")]
...
where
    Self: AsyncEngine,
{
    /// Asynchronously transmit data to the I3C controller.
    ///
    /// This function sends the contents of the provided buffer to the I3C controller. It
    ...
    /// Asynchronously receive data from the I3C controller.
    ///
    /// This function receives data from the I3C controller into the provided buffer. It
    ...
    ///
    /// The hardware automatically retries on NACK, so this method will keep
    /// waiting until the controller ACKs the IBI or the future is cancelled.
    /// If the future is dropped before completion, `SCTRL.EVENT` is restored to
    /// `NormalMode` to cancel the pending request.
    ///
    /// # Returns
    ///
    ...
    pub async fn async_send_ibi(&mut self, payload: &[u8]) -> Result<(), IOError> {
        ...
        // Release EVENT field back to normal and defuse the cancellation guard.
...
impl<T: Instance> typelevel::Handler<T::Interrupt> for InterruptHandler<T> {
    unsafe fn on_interrupt() {
        ...
        defmt::trace!("I3C target interrupt: status={:?}", status);
